...
export class Ground extends gfx.Mesh
{
    ...
    public reshapeGround(screenPath: gfx.Vector2[], groundStartPoint: gfx.Vector3,  groundEndPoint: gfx.Vector3, camera: gfx.Camera): void
    {
        ...
        const startToEnd = gfx.Vector3.copy(groundEndPoint);
        startToEnd.subtract(groundStartPoint);
        const Planenormal = gfx.Vector3.cross(startToEnd, gfx.Vector3.UP);
        Planenormal.normalize();
        const ProjectStrokePlane = new gfx.Plane(groundStartPoint, Planenormal);
        ...
        for(let i=0; i < screenPath.length; i++)
        {
            ...
            const intersection = ray.intersectsPlane(ProjectStrokePlane);
            if(intersection){
                silhouetteCurve.push(intersection);
                // silhouetteCurve[i].rotate(rotationInverse);
            }
        }
        ...
        {
            const closest_point = ProjectStrokePlane.project(this.vertices[i]);
            const d = ProjectStrokePlane.distanceTo(this.vertices[i]);
            const wd = Math.max(0, 1 - Math.pow((d/5), 2));
            const h = this.computeH(closest_point, silhouetteCurve, ProjectStrokePlane);
            if (h != 0){
